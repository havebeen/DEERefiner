...
%     RMSE = app.RMSE;
    ...
    for monteCarloSteps = 1:monteCarloIterationNumber
        ...
        if mod(monteCarloSteps, 3000)<=500
%             [DEERIfMovingCriterion, RMSEFirstPhase(monteCarloSteps+1)] = monteCarloMetropolisCriterionGenerator(monteCarloOldDistanceDistribution, monteCarloNewDistanceDistribution, targetDEERMaximalIndex, 5000);
            DEERIfMovingCriterion = 1;
            RMSEFirstPhase(monteCarloSteps+1) = RMSEFirstPhase(end);
            phiPsiAngleVariationAtLoop = phiPsiAngle*3;
        else
